...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/25 20:11:38 by satukoskine       #+#    #+#             */
/*   Updated: 2022/02/25 20:11:38 by satukoskine      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	vm_print_arena(t_arena arena, t_process *process_list)
{
	...
	while (i < MEM_SIZE)
	{
		...
		if (vm_is_process_pc(i, process_list) && arena.coloured_output)
			print("\x1b[31m%02x\x1b[0m", arena.mem.mem[i]);
		else
			print("%02x", arena.mem.mem[i]);
		i++;
